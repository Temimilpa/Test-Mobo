	...
	class Administrator extends CI_Controller{		
	  ...
	  // Log in Admin
		...
		public function orders($offset = 0)
		{
			...
			$data['orders'] = $this->Administrator_Model->get_orders(FALSE, $config['per_page'], $offset);
		...
		{
			// Pagination Config
			$config['base_url'] = base_url(). 'administrator/customers/';
			...
			$data['customers'] = $this->Administrator_Model->get_customers(FALSE, $config['per_page'], $offset);
		...
		{
			//$config['base_url'] = base_url(). 'administrator/config/';
			...
			//$this->pagination->initialize($config);
			$data['title'] = 'Costo de envío';

			$this->load->view('administrator/header-script');
	 	 	$this->load->view('administrator/header');
	  		$this->load->view('administrator/header-bottom');
	   		$this->load->view('administrator/costo_envio', $data);
	  		$this->load->view('administrator/footer');
		}
		
		public function update_costo_envio($page = 'costo_envio')
		{
			if (!file_exists(APPPATH.'views/administrator/'.$page.'.php')) {
		    show_404();
		   }
			...
			$data['title'] = 'Actualizar Costo de envío';

			
			$this->form_validation->set_rules('costo', 'Name', 'required');
			...
			if($this->form_validation->run() === FALSE){
				...
				$this->Administrator_Model->update_costo_envio();
				//Set Message
				$this->session->set_flashdata('success', 'El costo de envío ha sido actualizado satisfactoriamente');
				redirect('administrator/config/envio');
			}
